...
- **Story 3.2**: **Experience Timeline**: Build an interactive timeline for career history.
  - _Acceptance Criteria_: Expandable cards, "focus" blur effect on inactive items, bulleted job descriptions. `[DONE]`
- **Story 3.3**: **Projects Carousel**: Create an infinite, circular scrolling carousel for project showcases.
  - _Acceptance Criteria_: Snap scrolling, infinite loop illusion, hover effects on cards. `[DONE]`
- **Story 3.4**: **Contact Form**: Develop a functional contact form with validation.
  - _Acceptance Criteria_: Visual style matching "Experience" cards, success state updates the Status Bar. `[DONE]`
