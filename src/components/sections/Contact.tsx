  ...
  const checkLocation = async (address: string) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Call API to calculate distance and get travel stats (hides home location)
        try {
...
${formData.message}
  ...
  return (
    <section id="contact" className="py-20 px-8 max-w-4xl mx-auto">
      ...
      <div className="relative border-l-2 border-ide-border ml-3 md:ml-6">
        <motion.div 
        ...
        >
          {/* Timeline Icon */}
          <div className="absolute -left-[19px] top-0 w-10 h-10 rounded-full flex items-center justify-center border-4 border-ide-bg z-10 bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]">
          ...
          <div className="bg-ide-card-bg p-6 rounded-lg border border-ide-border hover:border-ide-accent/50 transition-colors group relative overflow-hidden">
            ...
            {submitted ? (
              ...
              >
                ...
                <p className="text-ide-text mb-6">
                  I've opened your email client with the message details. Please hit <strong>Send</strong> to complete the request.
                </p>
                {cooldown > 0 ? (
                      ...
                      <div className="flex flex-wrap gap-2 mb-2">
                        {technologies.map(tech => (
                          <span key={tech} className="bg-ide-activity-bar text-ide-accent text-xs px-2 py-1 rounded flex items-center gap-1">
                            {tech}
                            <X size={12} className="cursor-pointer hover:text-red-400" onClick={() => removeTech(tech)} />
                          </span>
                        ))}
                      </div>
                      <input 
