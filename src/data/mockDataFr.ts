export const experience = [
  {
    ...
    description: `*   Extension, maintenance et test des services legacy Java EE (monorepo SOA/SOAP avec front-ends JavaScript/React), en adaptant les suites de tests SoapUI et manuelles au fur et à mesure de l'évolution des services, et en ajoutant une couverture TestNG/Mockito pour les modifications.
*   Architecture et prise en charge de bout en bout de nouveaux services Spring Boot pour remplacer le legacy Java EE — architecture N-Tier propre avec mappage DTO strict, gestion centralisée des erreurs et traçage distribué basé sur MDC — exposant des API REST contract-first, compatibles HATEOAS (OpenAPI), soutenues par des tests d'intégration JUnit 5/MockMvc et des collections Postman avec tests automatisés.
*   Gestion de la couche de données avec JPA/Hibernate : PostgreSQL a remplacé les bases de données SQL legacy (principalement Oracle SQL) et MongoDB a introduit le NoSQL dans une pile technologique auparavant exclusivement SQL.
*   Refonte des front-ends JavaScript legacy en composants modernes et réactifs React/TypeScript, en optimisant l'architecture avec Effector (gestion d'état) et PrimeReact.
*   CI/CD : Construction de la plateforme legacy avec Gradle sur Jenkins et déploiement en test et production ; les nouveaux services ont été livrés via le Gradle Wrapper. Déploiement sur Azure Kubernetes (AKS) via des pipelines de build et release avec des charts Helm et NGINX ingress. Utilisation de Docker Compose pour le développement local. Migration du contrôle de version de GitLab vers Azure Repos. La plateforme legacy fonctionne avec 6 versions par an sur un tableau Kanban dans Jira, tandis que les nouveaux services sont passés à la livraison continue.
*   Modernisation du build parent Spring Boot hérité (OpenRewrite pour des mises à jour de version automatisées au-delà de la limite du BOM, un bot Renovate sur Azure pour les mises à jour des dépendances) et migration de la configuration des propriétés basées sur des dossiers vers des profils Spring.
*   Création d'un flux de travail augmenté par l'IA via IntelliJ IDEA (CodeGPT sur un LLM local Azure), Cursor et Windsurf pour documenter et analyser le code legacy, extraire les exigences et implémenter de nouveaux services maintenus 100 % propres selon SonarQube.
*   Orientation de la direction technique par le prototypage et l'évaluation de meilleures approches chaque fois que j'en trouvais une — en les démontrant dans du code fonctionnel et en animant des ateliers d'équipe (conception REST/API-first, React moderne et Next.js, OpenRewrite pour les mises à jour automatiques, Dependabot et une évaluation de Spring Cloud Gateway pour remplacer la passerelle personnalisée de DAT).`,
    tech: [
